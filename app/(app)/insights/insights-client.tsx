...
import { TrendingUp, BookOpen, Target, BarChart2, Calendar, Award, Zap } from "lucide-react";
...
type Trade = { [key: string]: any };
type JournalEntry = { entry_date: string; rating: number | null; rules_followed: boolean | null; mood: string | null };
type Account = { id: string; type: string | null; status: string | null };
...
export function InsightsClient({
  ...
  const availableTypes = useMemo(() => {
    ...
    allTrades.forEach((tr) => {
      ...
      if (acc?.type) s.add(acc.type);
    });
    return s;
  }, [allTrades, accounts]);

  const t = useMemo(() => {
    if (acctFilter === "all") return allTrades;
    return allTrades.filter((tr) => {
      const acc = accounts.find((a) => a.id === tr.account_id);
      return acc?.type === acctFilter;
    });
  }, [allTrades, acctFilter, accounts]);

  const e = entries;

  if (t.length === 0 && acctFilter === "all") {
    return (
  ...
  const bestTrade  = t.length > 0 ? t.reduce((a, b) => b.pnl > a.pnl ? b : a) : null;
  const worstTrade = t.length > 0 ? t.reduce((a, b) => b.pnl < a.pnl ? b : a) : null;
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="mb-6">
        ...
        {t.length > 0 && (
          <p className="text-sm text-muted-foreground mt-0.5">
            {fmtDateRange(firstDate)} — {fmtDateRange(lastDate)} · {t.length.toLocaleString()} trades · {tradingDays} trading days
          </p>
        )}
      </div>

      {/* Account type pill toggle */}
      <div className="mb-7 flex items-center gap-1.5 flex-wrap">
          ...
          >
            ...
            {tab.value !== "all" && (
              <span className="ml-1.5 opacity-60">
                {allTrades.filter((tr) => accounts.find((a) => a.id === tr.account_id)?.type === tab.value).length}
              </span>
            )}
          </button>
        ))}
      ...
      {/* ── Blown / Active account breakdown for eval/funded ── */}
      {(acctFilter === "eval" || acctFilter === "funded") && (() => {
        const accs = accounts.filter(a => a.type === acctFilter);
        ...
        const survivalRate = accs.length > 0 ? Math.round((active.length / accs.length) * 100) : 0;
        const blownTrades  = allTrades.filter(tr => {
          const acc = accounts.find(a => a.id === tr.account_id);
          return acc?.type === acctFilter && (tr as any).blown_account === true;
        });
        return (
          <div className="mb-7 rounded-xl border border-border bg-card p-4">
            <div className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground mb-3">
              {acctFilter === "eval" ? "Evaluation" : "Funded"} Account Overview
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="rounded-lg border border-green-500/30 bg-green-500/10 p-3">
                <div className="text-[10px] uppercase tracking-wide text-green-600 mb-1">Active</div>
                <div className="text-2xl font-black text-green-500">{active.length}</div>
                <div className="text-[10px] text-muted-foreground">of {accs.length} accounts</div>
              </div>
              <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-3">
                <div className="text-[10px] uppercase tracking-wide text-red-600 mb-1">Blown 💥</div>
                <div className="text-2xl font-black text-red-500">{blown.length}</div>
                <div className="text-[10px] text-muted-foreground">
                  {blownTrades.length > 0 ? `${blownTrades.length} blow-up trade${blownTrades.length > 1 ? "s" : ""}` : "no blow-up trades tagged"}
                </div>
              </div>
              <div className="rounded-lg border border-border p-3">
                <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">Survival Rate</div>
                <div className={`text-2xl font-black ${survivalRate >= 70 ? "text-green-500" : survivalRate >= 40 ? "text-amber-500" : "text-red-500"}`}>
                  {survivalRate}%
                </div>
                <div className="text-[10px] text-muted-foreground">accounts still live</div>
              </div>
              {blownTrades.length > 0 && (() => {
                const blownPnl = blownTrades.reduce((s, tr) => s + (tr.pnl - (tr.commission ?? 0)), 0);
                return (
                  <div className="rounded-lg border border-border p-3">
                    <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">Blown P&L</div>
                    <div className={`text-2xl font-black ${blownPnl >= 0 ? "text-green-500" : "text-red-500"}`}>
                      {blownPnl >= 0 ? "+" : ""}{new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(blownPnl)}
                    </div>
                    <div className="text-[10px] text-muted-foreground">on blow-up trades</div>
                  </div>
                );
              })()}
            </div>
      ...
      {t.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <BarChart2 className="h-10 w-10 text-muted-foreground mb-3 opacity-40" />
          <p className="text-muted-foreground text-sm">No {acctFilter} trades found</p>
        </div>
      ) : (<>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
        ...
        <StatCard label="Best Trade" value={bestTrade ? fmtMoney(bestTrade.pnl, true) : "—"} color="green" sub={bestTrade ? `${bestTrade.symbol} · ${bestTrade.trade_date}` : ""} />
        <StatCard label="Worst Trade" value={worstTrade ? fmtMoney(worstTrade.pnl, true) : "—"} color="red" sub={worstTrade ? `${worstTrade.symbol} · ${worstTrade.trade_date}` : ""} />
      </div>
