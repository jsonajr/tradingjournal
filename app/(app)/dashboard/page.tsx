...
export default async function DashboardPage() {
  ...
  const [{ data: trades }, { data: accounts }, { data: recentJournal }, { data: userSettings }] = await Promise.all([
    sb.from("trades").select("id, trade_date, symbol, direction, pnl, commission, r_multiple, account_id, setup, session, grade").eq("user_id", user.id).order("trade_date", { ascending: false }).range(0, 9999),
    sb.from("accounts").select("id, name, type, status").eq("user_id", user.id),
