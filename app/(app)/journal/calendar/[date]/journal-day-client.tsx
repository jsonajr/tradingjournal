...
import { ArrowLeft, Trash2, TrendingUp, TrendingDown, Star, CheckCircle2, XCircle, BookOpen, Zap, Clock, Edit, Pencil, Plus, FileText } from "lucide-react";
      ...
      {entry && (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          {entry.setups && entry.setups.length > 0 && (
            <Card>
              <CardHeader className="pb-2 pt-4 px-4"><CardTitle className="text-sm">Setups Traded</CardTitle></CardHeader>
              <CardContent className="px-4 pb-4">
                <div className="flex flex-wrap gap-1.5">{entry.setups.map(s => <Badge key={s} variant="secondary" className="text-xs">{s}</Badge>)}</div>
              </CardContent>
            </Card>
          )}
          {entry.sessions && entry.sessions.length > 0 && (
            <Card>
              ...
              <CardContent className="px-4 pb-4">
                <div className="flex flex-wrap gap-1.5">{entry.sessions.map(s => <Badge key={s} variant="secondary" className="text-xs">{s}</Badge>)}</div>
              </CardContent>
            </Card>
          )}
          {entry.improvement && (
