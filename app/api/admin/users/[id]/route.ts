...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  switch (action) {
    ...
    case "suspend": {
      ...
      // Deactivate any existing suspensions first
      await sb.from("cooldowns").update({ is_active: false })
        .eq("user_id", id).ilike("reason", "SUSPENDED:%");
      // Create new suspension — user can still log in but sees overlay
      await sb.from("cooldowns").insert({
        user_id: id, reason: `SUSPENDED: ${body.reason ?? "Admin suspension"}`,
        ends_at: endsAt, created_by: auth.profile.id, is_active: true,
      });
      await logAdminAction(auth.profile.id, "suspend_user", id, { hours, reason: body.reason });
      return NextResponse.json({ message: `Account suspended for ${hours} hours` });
    }
    case "set_plan": {
