...
const CHECKLIST_ITEMS = [
  ...
  "Review entry and exit",
  "Step away from charts",
  "Confirm I am not forcing another trade",
];
...
export function PostTradeReflection({ open, pnl, symbol, tradeId, onDismiss }: PostTradeReflectionProps) {
      ...
      <div className="w-full max-w-lg min-h-screen md:min-h-0 md:my-6 md:rounded-2xl bg-zinc-950 border border-white/10 overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className={cn("text-lg font-black leading-tight", isWin ? "text-green-400" : "text-red-400")}>
                {isWin ? "You just took a win. Stay disciplined." : "You just took a loss. Let's review your state."}
              </div>
              <div className="text-xs text-white/40 mt-0.5 uppercase tracking-wide">
                {symbol} · {pnl >= 0 ? "+" : ""}${Math.abs(pnl).toFixed(2)}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 py-5 space-y-6">
          ...
          >
            ...
            <span className="text-sm font-medium">I reviewed my state and I am ready to continue</span>
          </button>
          ...
          <Button
            ...
            className={cn(
              "w-full py-6 text-base font-bold tracking-wide transition-all",
              canContinue
                ? "bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg"
                : "bg-zinc-800 text-zinc-500 cursor-not-allowed"
            )}
          >
            {saving ? "Saving..." : canContinue ? "✓ Continue Trading" : "Complete all steps to continue"}
          </Button>
