...
import { LayoutDashboard, List, PlusCircle, CalendarDays, Settings, Shield, DollarSign, Upload } from "lucide-react";
...
export function MobileTabBar({ profile }: { profile: Profile }) {
  ...
  const TABS = [
    ...
    { href: "/eval",       label: "Eval",    icon: DollarSign },
        ...
        {TABS.map((t) => {
          ...
          return (
            <Link key={t.href} href={t.href} className={cn(
              "flex min-w-0 flex-1 flex-col items-center gap-0.5 py-2.5 text-xs font-medium transition-colors",
