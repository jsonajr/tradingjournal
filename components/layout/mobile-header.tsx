...
export function MobileHeader({ profile }: { profile: Profile }) {
  ...
  async function signOut() { if (!confirm("Are you sure you want to sign out?")) return; await supabase.auth.signOut(); router.push("/"); router.refresh(); }
  return (
    <header
    ...
    >
      ...
      <div className="flex items-center gap-1">
        <a href="https://discord.gg/uuyAxCavGd" target="_blank" rel="noopener noreferrer"
          className="flex h-8 w-8 items-center justify-center rounded-md text-[#5865F2] hover:bg-[#5865F2]/10 transition-colors">
